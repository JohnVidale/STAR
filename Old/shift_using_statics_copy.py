...
for i in array_select:
    ...
    # Loop over the three components and plot just the shifted stack for each on one figure with offsets
    comp_list = ["*Z", "*1", "*2"]
    vertical_offset = 2.0  # Amount to offset traces vertically between components
    fig_all, ax_all = plt.subplots(figsize=(10, 8))

    # For each component, process the traces, compute the shifted stack, then plot with an offset.
    for comp in comp_list:
        # Select traces matching the desired component within the current fixed window
        ...
        # Demean, taper, filter, and normalize each trace for this component
        for tr in st_comp:
            tr.detrend(type="demean")
            tr.taper(max_percentage=0.05, type="cosine")
            tr.filter('bandpass', freqmin=min_freq, freqmax=max_freq, corners=4, zerophase=False)
            norm_factor = np.max(np.abs(tr.data))
            if norm_factor != 0:
                tr.data = tr.data / norm_factor

        # Get parameters (assuming all traces have same npts and sample_rate)
        sample_rate = st_comp[0].stats.sampling_rate
        nstack = int(sample_rate * (end_time - start_time))
        
        # Initialize stack for shifted traces only
        stack_shifted = np.zeros(nstack)
        count = 0
        
        # Loop over traces and sum the shifted (aligned) traces.
        for tr in st_comp:
            unshifted = tr.data[:nstack]
            
            # Retrieve lag information from data_df using the station ID
            data_info = data_df[data_df['station'] == int(tr.stats.station)]
            if data_info.empty:
                continue
            select_trace = bool(data_info.iloc[0]['select'])
            if not select_trace:
                continue
            lag = int(data_info.iloc[0]['lag3'])
            # Check if lag is within reasonable bounds; otherwise use zero.
            ...
            # Compute the shifted (aligned) trace; using np.roll for now (wrap-around)
            # If you prefer non-wrapping behavior, use your custom roll_with_zeros() function.
            shifted = np.roll(unshifted, -lag)
            stack_shifted += shifted
            count += 1
        
        if count > 0:
            stack_shifted /= count  # Average the stack
            
            # Build a time axis relative to the pick (analysis window defined from start_time to end_time)
            t_stack = np.linspace(start_time, end_time, nstack)
            # For visual separation, add a vertical offset depending on the component:
            if comp == "*Z":
                offset = 0
            elif comp == "*1":
                offset = vertical_offset
            elif comp == "*2":
                offset = 2 * vertical_offset
            
            ax_all.plot(t_stack, stack_shifted + offset, label=f"Shifted Stack {comp}")
        else:
            print(f"No selected traces for stacking for component {comp} in Array {i}")

    ax_all.set_xlabel("Time (s) relative to pick")
    ax_all.set_ylabel("Normalized Amplitude + offset")
    ax_all.set_title(f"Array {i}: Shifted Stacks for Components Z, 1, and 2")
    ax_all.legend(loc="upper right", fontsize="small")

    # Optionally, save the figure for the combined stacks.
    stack_all_fig_path = os.path.join(save_dir, f"Array{i}_All_Shifted_Stacks.png")
    fig_all.savefig(stack_all_fig_path)
    print(f"Saved combined shifted stacks for Array {i} to {stack_all_fig_path}")
