...
# Keisuke 4/11/2025, further aligns the waveforms at 4-40 Hz,
...
def compute_lag(ref, d, win_start, win_end):
    move_limit = 6 # maximum number of time steps to allow shift
    ...
    return np.argmax(corr) - move_limit

def read_align_plot2(star_volume_path, event_file, channel, array_select, align_phase, evid):

    # Define offsets (in seconds) relative to the picked phase arrivel time
    start_time = -0.3   # seconds for start of analysis window
    end_time   = 0.5   # seconds for end   of analysis window
    duration = end_time - start_time
    extra_time_pre = 30 # lengthen the window by this amount before start so that shifting can be applied
    extra_time_post = 30 # lengthen the window by this amount after end so that shifting can be applied
    short_win_pre  =  0.1 # seconds for short cross-correlation window pre-pick
    short_win_post =  0.3 # seconds for short cross-correlation window post-pick
    ...
    data_file = "Results/output3_" + evid + "_" + input_array_string + ".csv"
    ...
    for i in array_select:
        ...
        st_window = overlapping_traces.slice(starttime = abs_pick - extra_time_pre, endtime = abs_pick + duration + extra_time_post)
        ...
        search_win_start = int(max(0,    sample_rate * (-start_time + extra_time_pre - search_win_pre )))
        search_win_end   = int(min(npts, sample_rate * (-start_time + extra_time_pre + search_win_post)))
        ...
        # limit to times that exist in case correlation window is too large
        win_start = int(max(0,    sample_rate * (extra_time_pre - start_time - short_win_pre  + rising_time_shift)))
        win_end   = int(min(npts, sample_rate * (extra_time_pre - start_time + short_win_post + rising_time_shift)))
        plot_start = int(sample_rate * (extra_time_pre + rising_time_shift))
        plot_end   = int(sample_rate * (extra_time_pre + rising_time_shift + duration))
        ...
        for tr in st_comp:
            ...
            rolled_data = np.roll(d, -lag)

            # Compute unnormalized cross-correlation on the normalized segments
            lag1 = lag + compute_lag(ref, rolled_data, win_start, win_end)
        ...
        for tr in st_comp:
            ...
            # Compute unnormalized cross-correlation on the normalized segments
            lag2 = lag + compute_lag(aligned_stack, rolled_data, win_start, win_end)
        ...
        for j, chunk_start in enumerate(range(0, num_traces, chunk_size)): # make each subplot
            ...
            for idx, tr in enumerate(chunk):
                ...
                rolled_data = np.roll(x, -lag)

                # Compute unnormalized cross-correlation on the normalized segments
                lag3 = lag + compute_lag(selected_aligned_stack, rolled_data, win_start, win_end)
                ...
                cut_data = aligned_data[plot_start:plot_end]
                cut_ref = selected_aligned_stack[plot_start:plot_end]
                ...
                if np.linalg.norm(aligned_window) == 0 or np.linalg.norm(ref_window) == 0:
                    ...
                    r_window       = np.dot(aligned_window, ref_window) / (np.linalg.norm(aligned_window) * np.linalg.norm(ref_window))
                if np.linalg.norm(cut_data)   == 0 or np.linalg.norm(selected_aligned_stack)        == 0:
                    r_whole = 0
                else:
                    r_whole = np.dot(cut_data,   cut_ref)        / (np.linalg.norm(cut_data)   * np.linalg.norm(cut_ref))
                ...
                ax.text((times + start_time + p_traveltime + rising_time_shift)[0], offset, f"{r_window:.2f}",
                        fontsize=8, color="blue", verticalalignment="top")
                ax.text((times + start_time + p_traveltime + rising_time_shift)[-1], offset, f"{r_whole:.2f}",
        ...
        ax_stack.plot(t_axis + start_time + p_traveltime - extra_time_pre,
                    ref, color="red", lw=1, label="Original stack")
        # Plot the overall aligned stack (after first round of alignment)
        ax_stack.plot(t_axis + start_time + p_traveltime - extra_time_pre,
                    aligned_stack - stack_offset, color="green", lw=1, label="Aligned stack")
        # Plot the "good" aligned stack (selected traces)
        ax_stack.plot(t_axis + start_time + p_traveltime - extra_time_pre,
                    selected_aligned_stack - 2 * stack_offset, color="black", lw=1, label="Good stack")
        ...
        ax_short.set_xlabel("Time (s) relative to earthquake origin")
    ...
    #%% save lag to csv
    # output_file1 = "Results2/output1_" + evid + "_" + array_string + ".csv"
    ...
    #     for rec in results1:
    #         writer.writerow(rec)
    # output_file2 = "Results2/output2_" + evid + "_" + array_string + ".csv"
    ...
    #     for rec in results2:
    #         writer.writerow(rec)
    output_file3 = "Results2/output3_" + evid + "_" + array_string + ".csv"
...
if __name__=="__main__":
    ...
    evid = "ci40995120" #"ci40789071"

    read_align_plot2(star_volume_path, event_file, channel, array_select, align_phase, evid)
