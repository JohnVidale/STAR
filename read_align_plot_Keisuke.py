...
# JV 3/29/2025, fixed by Ruoyan 4/1/2025 and Keisuke 4/10/2025
...
short_win_pre  =  0.5 # seconds for short cross-correlation window pre-pick
short_win_post =  0.5 # seconds for short cross-correlation window post-pick
search_win_pre  =  1 # seconds to search for peak near arrival time
...
for i in array_select:
    ...
    peak_amp = np.max(search_window)
    peak_index = np.argmax(search_window > peak_amp / 10)
    peak_time_shift = peak_index / sample_rate - search_win_pre
    print(f"!    peak_time_shift = {peak_time_shift}")

    # limit to times that exist in case correlation window is too large
    win_start = int(max(0,    sample_rate * (-start_time - short_win_pre  + peak_time_shift)))
    win_end   = int(min(npts, sample_rate * (-start_time + short_win_post + peak_time_shift)))
#    win_start_extended = int(max(0,    sample_rate * (-start_time - short_win_pre  * 3)))
#    win_end_extended   = int(min(npts, sample_rate * (-start_time + short_win_post * 3)))
    ...
    for j, chunk_start in enumerate(range(0, num_traces, chunk_size)): # make each subplot
        ...
        for idx, tr in enumerate(chunk):
            ...
            ##TEMP Get lag data
            result1_data = next(item for item in results1 if item["station"] == station_id)
            result1_lag = result1_data["lag1"]
            result2_data = next(item for item in results2 if item["station"] == station_id)
            result2_lag = result2_data["lag2"]
            
            x = tr.data[:npts]
            x_window = x[win_start:win_end]
#            x_window = x[win_start_extended:win_end_extended]
            ref_window = ref[win_start:win_end]
            
            corr = np.correlate(x_window, ref_window, mode="full")
            lag3 = np.argmax(corr) - (len(x_window) - 1)
#            corr = np.correlate(x_window, ref_window, mode="valid")
#            lag3 = np.argmax(corr) - (len(ref_window) - 1)
            aligned_data = np.roll(data, -lag3)
#            aligned_data = np.roll(data, 0)
#            aligned_data = np.roll(data, -result2_lag)
